...
@click.option("--enable-expert-parallel/--no-enable-expert-parallel", type=bool, default=False)
...
def sample_logits(  # noqa: C901
    ...
    tok = transformers.AutoTokenizer.from_pretrained(tokenizer or model,
    ...
    if cfg.d != tok_vocab_size:
        if auto_vocab_size:
            ...
            logging.warning(f"Automatically set compressor vocab size to {tok_vocab_size}")
        elif cfg.d < tok_vocab_size:
            logging.error("Compression config has too small vocabulary size!")
            logging.error(f"cfg.d: {cfg.d}, effective tokenizer vocab size: {tok_vocab_size}")
            sys.exit(-1)
        elif (abs(cfg.d - tok_vocab_size) > 32):  # allow a little wiggle room for common padding
            logging.warning(
    ...
    with open(os.path.join(output, "compression_config.yaml"), "w", encoding="utf-8") as f:
    ...
    def process_and_write_sample(
        ...
        compressed_logprobs_list = (row_out["compressed_logprobs"].cpu().squeeze(0).tolist())
        bytepacked_indices_list = (row_out["bytepacked_indices"].cpu().squeeze(0).tolist())
...
def process_prompt_logprobs(  # noqa: C901
        prompt_logprobs: PromptLogprobs, k: int) -> tuple[torch.LongTensor, torch.Tensor]:
    ...
    if isinstance(prompt_logprobs, FlatLogprobs):
        ...
        if seq_ids:
            ...
            top_indices[seq_idx_tensor, rank_idx_tensor] = torch.tensor(token_ids_to_copy,
                                                                        dtype=top_indices.dtype,
                                                                        device=top_indices.device)
            top_values[seq_idx_tensor, rank_idx_tensor] = torch.tensor(logprobs_to_copy,
    ...
    # Slow path: handle legacy list format
        ...
        top_values = torch.full((num_prompt_tokens, k),
                                fill_value=float("-inf"),
                                dtype=torch.float32)
