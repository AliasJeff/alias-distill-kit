...
class LogitCompressor:
    ...
    def __init__(self, config: LogitCompressionConfig):
        self.config = config
        self.vocab_index_bits = (torch.log2(
            torch.tensor(self.config.vocab_size, dtype=torch.float32)).ceil().item())
        self._validate_config()
        self._setup_polynomial_terms()

    def _validate_config(self):
        assert self.config.exact_k <= self.config.k, ("exact_k must be less than or equal to k")
    ...
    def compress_from_sparse(self, top_indices: torch.LongTensor, top_values: torch.Tensor):
    ...
    def compress(self, logits: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    def decompress_to_sparse(
        ...
        approx_logits = torch.sum(
            self.X.to(coeffs.device, coeffs.dtype) * coeffs.to(dtype=self.X.dtype).unsqueeze(-2),
            dim=-1,
        )
    ...
    def decompress(
        ...
        top_indices, top_values = self.decompress_to_sparse(packed_indices, exact_values, coeffs)
    ...
    def bytes_per_token(self) -> int:
        ...
        return (self.config.exact_k * 2 + self.config.polynomial_degree *
                (self._str_to_dtype(self.config.term_dtype).itemsize) + index_bytes)
...
def pack_tensor(x: torch.Tensor, bits: int) -> torch.Tensor:
    ...
    pad_size = (elements_per_64bits - x.shape[-1] % elements_per_64bits) % elements_per_64bits
...
def unpack_tensor(packed: torch.Tensor, original_size: torch.Size, bits: int) -> torch.Tensor:
