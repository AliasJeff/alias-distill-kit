...
class LogprobCompressor:
    ...
    def __init__(
        ...
        if config is not None and legacy_config is not None:
            raise ValueError("At most one of `config` or `legacy_config` should be provided.")
        self.config = config
    ...
    def compress_from_sparse(self, indices: torch.LongTensor,
                             logprobs: torch.Tensor) -> dict[str, torch.Tensor]:
        if self.legacy_compressor is not None:
            packed_indices, exact_values, coeffs = (self.legacy_compressor.compress_from_sparse(
                indices, logprobs))
            return {
    ...
    def decompress_to_sparse(
        ...
        if "top_values" in row and "token_ids" in row:
            ...
            return self.legacy_compressor.decompress_to_sparse(row["packed_indices"],
                                                               row["exact_values"], row["coeffs"])
