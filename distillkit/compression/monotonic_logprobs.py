...
def polynomial_terms(
    ...
    assert all(isinstance(i, (int, SpecialTerm))
               for i in terms), ("terms must be a list of integers or SpecialTerm instances")
    if normalize_t:
...
def error_diffuse_and_quantize(
    ...
    scale_factor = torch.where(max_abs_val == 0, torch.ones_like(max_abs_val),
                               max_abs_val / max_quant_abs)
...
def quantize_naive(
    ...
    quant_min, quant_max = _get_quantize_range(element_bits)
    quantized_values = (torch.round(scaled_vals).clamp(quant_min, quant_max) - quant_min).to(
        torch.long)
...
def compress_monotonic_logprobs(
    ...
    if config.exact_k > 0:
        ...
        chunks.append(exact_values.view(torch.uint8).reshape(
            *logprobs.shape[:-1],
            -1,
        ), )
    ...
    for bin in config.residual_bins:
        ...
        if config.error_diffusion:
            scale, scaled, error_buffer = error_diffuse_and_quantize(values, bin.element_bits,
                                                                     bin.scale_dtype.dtype(),
                                                                     error_buffer)
        else:
            scale, scaled, error_buffer = quantize_naive(values, bin.element_bits,
                                                         bin.scale_dtype.dtype(), error_buffer)
...
def decompress_monotonic_logprobs(
    ...
    if config.exact_k > 0:
        ...
        exact_values = torch.empty((*bytes.shape[:-1], 0), dtype=out_dtype, device=device)
    ...
    if config.polynomial_terms and len(config.polynomial_terms) > 0:
        ...
        coeffs = coeff_part.view(dtype=term_dtype_torch).reshape(*remaining_bytes.shape[:-1],
                                                                 terms_count)
    ...
    for bin in config.residual_bins:
        ...
        scale = scale_part.view(dtype=scale_dtype_torch).reshape(*remaining_bytes.shape[:-1], 1)
    ...
    if use_residual and residuals:
        ...
        if sum_bin_elems < approx_terms:
            residual = torch.nn.functional.pad(residual, (0, approx_terms - sum_bin_elems))
    else:
        residual = torch.zeros((*remaining_bytes.shape[:-1], approx_terms),
                               dtype=out_dtype,
                               device=device)

    # Compute polynomial approximation
    if (coeffs is not None and config.polynomial_terms and len(config.polynomial_terms) > 0):
        X = polynomial_terms(
