...
class DistributionQuantizationConfig(BaseModel):
    ...
    polynomial_terms: list[SpecialTerm | int] | None = Field(
        None,
        description=
        "Terms to use in the polynomial approximation. Integer values represent power terms, "
    ...
    normalize_t: bool = Field(default=False, description="Map t to [0,1] instead of [0,k].")
    ...
    def check_valid(self) -> "DistributionQuantizationConfig":
        ...
        if bin_elems > approx_terms:
            raise ValueError("Sum of num_elements in residual_bins must be <= k - exact_k")
        return self
    ...
    def total_bits(self) -> int:
        ...
        vocab_index_bits = int(torch.log2(torch.tensor(self.d, dtype=torch.float32)).ceil().item())
