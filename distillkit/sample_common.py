...
def do_chat_template(row: dict, tokenizer: transformers.PreTrainedTokenizerBase):  # noqa: C901
    if "text" in row:
        return row["text"]
...
def load_preprocess_data(
    *,
    ...
    ds = ds.map(
        lambda x: {
            "input_ids": truncate_tokens(x["text"], tokenizer, max_seq_len, add_extra_pad_token)
        },
        num_proc=64,
...
def truncate_tokens(text: str, tokenizer, max_seq_len: int, add_extra_pad_token: bool = False):
    tokens: torch.Tensor = tokenizer(text, return_tensors="pt")["input_ids"][0]
    if (add_extra_pad_token and tokens.shape[0] < max_seq_len and tokens.shape[0] > 0
...
class StreamingParquetWriter:
    ...
    def _ensure_writer_open(self):
        ...
        if self.pq_writer is None:
            file_path = os.path.join(self.output_path, f"data_{self.file_index}.parquet")
...
def legacy_compressed_logit_schema() -> pyarrow.Schema:
    return pyarrow.schema([
        ...
        pyarrow.field("packed_indices", pyarrow.list_(pyarrow.list_(pyarrow.uint64()))),
        pyarrow.field("exact_values", pyarrow.list_(pyarrow.list_(pyarrow.float32()))),
