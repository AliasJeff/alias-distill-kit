...
class SignalSource(ABC):
    ...
    def get_signal(self,
                   batch: dict[str, Any],
                   return_hidden_states: bool = False) -> TeacherSignal:
        ...
...
class OfflineSignalSource(SignalSource):
    ...
    def get_signal(self, batch: dict[str, Any], return_hidden_states: bool = False) -> SparseSignal:
        if return_hidden_states:
            raise RuntimeError("Hidden states requested but signal source is precomputed logits")
        with torch.no_grad():
...
class OnlineSignalSource(SignalSource):
    ...
    def get_signal(self,
        ...
        if self.sparsify_top_k is not None:
            logprobs = torch.log_softmax(teacher_outputs.logits[..., :vocab_size], dim=-1)
