...
def get_max_seqlen_in_batch(attention_mask):
    ...
    for i in range(1, max_num + 1):
        counts.append(torch.sum(attention_mask == i,
                                axis=-1))  # shape: B, count length of data point maksed with i
...
def get_unpad_data(attention_mask):
    seqlens_in_batch = get_max_seqlen_in_batch(
        attention_mask)  # attention_mask.sum(dim=-1, dtype=torch.int32)
...
def load_balancing_loss_func(
    ...
    if attention_mask is None:
        ...
        expert_attention_mask = (new_attention_mask[None, :, :, None, None].expand(
        ...
        router_per_expert_attention_mask = (new_attention_mask[None, :, :, None].expand(
        ...
        router_prob_per_expert = torch.sum(routing_weights * router_per_expert_attention_mask,
                                           dim=0) / torch.sum(router_per_expert_attention_mask,
                                                              dim=0)
