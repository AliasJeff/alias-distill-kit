...
def sparse_logistic_ranking_loss(
    ...
    student_logit_diff_k_minus_l = student_logits_at_targets.unsqueeze(
        -1) - student_logits_at_targets.unsqueeze(-2)  # [B, S, k_sparse, k_sparse]
    ...
    if sequence_mask is None:
        active_sequence_mask = torch.ones_like(sum_pair_loss_per_pos,
                                               dtype=torch.bool,
    ...
    num_contributing_pairs = (valid_preference_pair_mask.float() *
                              active_sequence_mask.float().unsqueeze(-1).unsqueeze(-1)).sum()
...
class LogisticRankingLoss(LossFunctionBase):
    ...
    def __call__(
        ...
        if isinstance(signal, DenseSignal):
            raise RuntimeError("Logistic ranking loss is not supported for dense predictions")
