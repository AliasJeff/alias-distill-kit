...
def sparse_tvd_inner(
    ...
    tvd_sparse_terms_sum = torch.sum(torch.abs(sparse_teacher_probs - sparse_student_probs),
                                     dim=-1)  # Shape: (B, S)
    ...
    if missing == MissingProbabilityHandling.SYMMETRIC_UNIFORM:
        ...
        tvd_missing_contrib = torch.abs(teacher_missing_prob_mass -
                                        student_missing_prob_mass)  # B, S
