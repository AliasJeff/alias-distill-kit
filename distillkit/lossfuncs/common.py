...
def get_target_logprobs(
    ...
    if log_target:
        ...
        if temperature_change:
            logits = values_in * (target_generation_temperature / distillation_temperature)
        else:
            logits = values_in
        sparse_max = torch.max(logits, dim=-1, keepdim=True).values
        sparse_lse = (torch.logsumexp(
            (logits - sparse_max).to(torch.float32), dim=-1, keepdim=True) + sparse_max).to(
                values_in.dtype)
...
def get_logprobs(
    ...
    if (not log_target) and (missing != MissingProbabilityHandling.ZERO):
        raise ValueError("For log_target=False (teacher inputs are logits), "
