...
def sparse_jsd_inner(
    ...
    if missing == MissingProbabilityHandling.SYMMETRIC_UNIFORM:
        ...
        student_total_missing_prob_mass = (1.0 - student_prob_sum_sparse).clamp(min=0.0)  # B, S
...
def dense_js_div(
    ...
    kl_student_mixture = torch.sum(student_probs * (student_log_probs - mixture_log_probs), dim=-1)
    kl_teacher_mixture = torch.sum(teacher_probs * (teacher_log_probs - mixture_log_probs), dim=-1)
    ...
    def __call__(
        ...
        if num_items_in_batch is None:
            if mask is not None:
                ...
                num_items_in_batch = (student_outputs.logits.shape[0] *
