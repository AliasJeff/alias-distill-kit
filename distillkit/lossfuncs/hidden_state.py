...
def compute_hs_loss(
    ...
    assert hidden_state_mapping is not None, ("Hidden state losses require HiddenStateMapping")
    assert len(
        hidden_state_mapping.layer_mapping) > 0, ("No layers specified in hidden state mapping")
    ...
    for i, (student_layer_idx, teacher_layer_idx) in enumerate(hidden_state_mapping.layer_mapping):
...
class HiddenStateCosineLoss(LossFunctionBase):
    ...
    def __call__(
        ...
        return compute_hs_loss("cosine", student_outputs, signal, mask, hidden_state_mapping)
...
class HiddenStateMSELoss(LossFunctionBase):
    ...
    def __call__(
        ...
        return compute_hs_loss("mse", student_outputs, signal, mask, hidden_state_mapping)
