...
def pack_pass(data: dict[str, list], max_len: int) -> dict[str, list]:  # noqa: C901
    # pack a set of rows into a (hopefully smaller) set of batches
    # truncate rows that are too long
    row_lens = [(len(input_ids), idx) for (idx, input_ids) in enumerate(data["input_ids"])]
    row_lens.sort()
    ...
    while row_lens and row_lens[-1][0] >= max_len:
        ...
        for key in data:
            ...
            assert len(
                data_out[key][-1]) == max_len, (f"{key} {len(data_out[key][-1])} != {max_len}")
...
def pack_logits_cli(
    ...
    if os.path.exists(dataset):
        ...
        if split is not None:
            assert isinstance(ds, datasets.DatasetDict), ("Expected a DatasetDict, got a Dataset")
            ds = ds[split]
    else:
