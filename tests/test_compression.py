...
class CompressionEvalResult(BaseModel):
    ...
    def __str__(self):
        return (f"Bytes per token: {self.bytes_per_token:.4f}\n"
...
def generate_test_logprobs(
    ...
    if max_seq_len < 1 or max_seq_len > model.config.max_position_embeddings:
        print(f"Clamping max_seq_len from {max_seq_len} to {model.config.max_position_embeddings}")
    ...
    ds = (datasets.load_dataset(
        dataset,
        split="train",
    ).shuffle(seed=42).select(range(samples)))
    ds = ds.map(
        lambda x: tokenizer(
    ...
    def _generate_rows():
        with torch.no_grad():
            for i_0 in tqdm.tqdm(range(0, len(ds), batch_size), desc="Generating logprobs"):
...
def jensen_shannon_divergence(log_p: torch.Tensor,
                              log_q: torch.Tensor,
    ...
    del m
    jsd = 0.5 * (torch.nn.functional.kl_div(log_m, p, log_target=False, reduction="none") +
...
def eval_compression_quality(
    ...
    compressor = (LogprobCompressor(
        config=config if isinstance(config, DistributionQuantizationConfig) else None,
        legacy_config=(config if isinstance(config, LegacyLogitCompressionConfig) else None),
    ...
    for i_0 in tqdm.tqdm(range(0, len(logprob_rows), batch_size), desc="Evaluating"):
        ...
        mask = (torch.stack([row["attention_mask"] for row in batch], dim=0) > 0).to(device)
    ...
    for mph in [
        ...
        for config in TEST_CONFIGS:
            ...
            if config == "bf16":
                ...
                report.append({
                    "kind": ("legacy" if isinstance(config, LegacyLogitCompressionConfig) else
                             "distrib_quant"),
                    "config":
                    config.model_dump(mode="json"),
                    "missing_prob_handling":
                    mph.value,
                    "result":
